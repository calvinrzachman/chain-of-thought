...
How difficult it is for any one attempt to succeed? The Bitcoin Network sets the likelihood that your next swing of the pick axe strikes gold. The Bitcoin Network/software is able to use its blockchain (NOTE: There are thousands of copies of the blockchain being independently maintained, but I am able to refer to a the blockchain as a single source of truth/information because of consensus) to determine the historic rate of block creation/transaction processing/Bitcoin issuance. Bitcoin adapts the energy it requires such that issuance and . Like the nervous system regulating a heart beat.
Bitcoin software regulates its fiscal/monetary policy in this self-referential way. With "the" blockchain as a single source of truth (since they all agree on the state of the system) the nodes can adjust their proof of work difficulty requirement in tandem/lock-step.
To be clear: Individual Bitcoin Network participants set their own difficulty, but because they share a consistent view on the system state, they are able to reach agreement on the selection of difficulty. (Question: What happens if someone chooses their own difficulty -> Recall what proof of work does.
