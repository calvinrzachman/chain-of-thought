...
The hash H(m) of a message m is deterministic (ie: non-random). In order to calculate hashes of a message repeatedly until we successfully find a hash which satisfies our Proof of Work criterion outlined above, we need to introduce some malleability to the message. That is we need to slightly modify our message, m, such that we can calculate a new hash `h' = H(m')` This is achieved by adding a number to the end of your message, recalculating the hash of your new message, and incrementing. This number is known as a nonce (number used only once) and it is the source of malleability for our messages which we modify until we can find a hash we consider to be a valid proof of work.
