...
- Domain where there exists a tradeoff between payment reliability and privacy
    - "Free Probing": sending payments with hash preimage known only to sender allows network participant to probe channel balances. This can be used to improve the probability that a payment will succeed but is quite clearly at odds with maintaining the privacy of channel balances. Indeed one could argue why keep the channel balances private in the first place if they're only to be undermined by this style of probing.
